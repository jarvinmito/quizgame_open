...
var App = (function(){
	...
	var initSetup = function(){
		...
		if( configMap.isStatic ){
			...
			var match_details;
			if( configMap.engage_match === null ){
				match_details = localStorage[configMap.plstore.match];
			}else{
				match_details = configMap.engage_match;
			}
	...
	var bind = function(){
		...
		if( $('#player__answer').length ){
			$('#player__answer').on('keypress', function(e){
				var key = e.which || e.keyCode;
				if( key === 13 ){
					$('.game-button--ttsubmit').trigger('click');
				}
			});
		}
	...
	var checkAnswer = function(){
		// console.log(input);
		var $oended = $('.open-ended'),
			$pinput = $('#player__answer'),
			$danswer = $('.open-ended__answer'),
			answer = $pinput.val().toLowerCase(),
		...
		if(answer === qanswer){
			...
			$oended.removeClass('is-invalid is-wrong');
			$oended.addClass('is-correct');
		}else{
			pts = 0;

			// Play sfx
			// configMap.audio.sfx.sfx_incorrect.play();
			$oended.removeClass('is-invalid is-correct');
			$oended.addClass('is-wrong');
		}

		$danswer.html(qanswer.toUpperCase());
	...
	var callApi1 = function(){
		...
		$.ajax({
			...
			success : function(result){
				...
				configMap.engage_match = jsonToString(result);
		...
		$.ajax({
			...
			success : function(response){
				if( response.response_code === 0){
					...
					// if( result.qs.length ){
					// 	var qslen = result.data.qs.questions.length;
					// 	for(var q = 0 ; q < qslen ; q++){
					// 		result.data.qs.questions[q].user_answer = null;
	...
	var callApi10 = function(exception){
		...
		console.log(data);
		// remove Modal
		var screens = ['waiting-rm','result', 'waiting-ch'];
				...
				if( result.match.status ){
					...
					if( status == 'ongoing' ){
						...
						if( configMap.waitTimer.currentTime >= 30){
							...
							if( configMap.currentScreen === 'waiting-rm'){
								destroyWaitingRematch();
								renderResultButtons(true);
							}
	...
	var renderQuestion = function(){
		...
		var data = q;
		data.round = configMap.currentRound;
		data.maxround = configMap.qs.questions.length;
		...
		$container.html(App.Templates['question']({ data : data, imagepath : qimagePath }));
	...
	var renderOptions = function(){
		...
		var q = configMap.qs.questions[configMap.currentRound - 1],
			$container = jqueryMap.$ingame.find('.ingame__options');
		...
		$container.append(App.Templates['options']({}));
		bind();
		...
		if( configMap.gameTimer.points % 5 === 0){
			...
			$.ajax({
				...
				success : function(result){
					...
					if( result[op.place].isactive === "false" ){
						configMap.modal.title = "Uh oh...";
						configMap.modal.message = "Your opponent has left the game.";
						configMap.modal.isActive = true;
						configMap.modal.buttons = {
								"back" : { "caption" : "Close", "class" : "pop-up__leave--yes is-yellow" }
							};
						renderModal();
					}
			...
			$.ajax({
				...
				success : function(result){
					if( configMap.matchStatus === 'find' ){
						...
						console.log(' Questions : ', configMap.qs.questions );
					}
					ajxCallback(result);
				},
				error : function(xhr, t, m){
